...
/**
 * Extract all entities from a parsed markdown document using regex patterns
 */
export const extractEntitiesRegex = (content: MarkdownContent, filename: string): AnyEntity[] => {
    ...
    entities.push(...extractNPCs(content, sessionInfo?.session_number));

    // Extract locations
    entities.push(...extractLocations(content, sessionInfo?.session_number));

    // Extract items
    entities.push(...extractItems(content, sessionInfo?.session_number));

    // Extract quests/missions
    entities.push(...extractQuests(content, sessionInfo?.session_number));
...
/**
 * Extract NPC entities from content using regex patterns
 */
const extractNPCs = (content: MarkdownContent, sessionNumber?: number): NPC[] => {
    const npcs: NPC[] = [];
    ...
    npcPatterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(text)) !== null) {
            ...
            if (foundNames.has(name.toLowerCase()) || isCommonWord(name) || name.length < 3) {
                continue;
            }
            ...
            if (match[2] && isRole(match[2])) {
                npc.role = match[2].toLowerCase();
            }

            if (match[4] && isClass(match[4])) {
                npc.tags = npc.tags || [];
...
/**
 * Extract location entities from content using regex patterns
 */
const extractLocations = (content: MarkdownContent, sessionNumber?: number): Location[] => {
...
/**
 * Extract item entities from content using regex patterns
 */
const extractItems = (content: MarkdownContent, sessionNumber?: number): Item[] => {
    const items: Item[] = [];
    const text = content.text;
...
/**
 * Extract quest entities from content using regex patterns
 */
const extractQuests = (content: MarkdownContent, sessionNumber?: number): Quest[] => {
    const quests: Quest[] = [];
    const text = content.text;
...
/**
 * Check if a word is a race/species
 */
const isRace = (word: string): boolean => {
...
/**
 * Check if a word is a character class
 */
const isClass = (word: string): boolean => {
...
/**
 * Check if a word is a role/occupation
 */
const isRole = (word: string): boolean => {
 ...
 * @deprecated Use extractEntitiesRegex function instead
 */
export class EntityExtractor {
    extractEntities(content: MarkdownContent, filename: string): AnyEntity[] {
        return extractEntitiesRegex(content, filename);
    }
}