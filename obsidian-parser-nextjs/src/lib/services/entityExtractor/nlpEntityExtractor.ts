...
export const extractEntities = (content: MarkdownContent): AnyEntity[] => {
    const doc = nlp(content.text) as CompromiseDocument;
 ...
 */
const extractNPCs = (doc: CompromiseDocument, content: MarkdownContent, sessionNumber?: number): NPC[] => {
    const npcs: NPC[] = [];
 ...
 */
const extractLocations = (doc: CompromiseDocument, content: MarkdownContent, sessionNumber?: number): Location[] => {
 ...
 */
const extractItems = (doc: CompromiseDocument, _content: MarkdownContent, sessionNumber?: number): Item[] => {
    const items: Item[] = [];
 ...
 */
const extractQuests = (doc: CompromiseDocument, _content: MarkdownContent, sessionNumber?: number): Quest[] => {
    const quests: Quest[] = [];
    ...
    const questCandidates = [
        ...questPatterns.out('array') as string[],
        ...missionPatterns.out('array') as string[]
    ];
 ...
 */
const getContextAroundName = (doc: CompromiseDocument, name: string): string => {
    // Get sentences containing the name
    const sentences = doc.sentences().filter((s: CompromiseSentence) => s.text().includes(name));
    return sentences.out('text') as string;
};
 ...
 * Determine location type from context
 */
const determineLocationType = (doc: CompromiseDocument, locationName: string): LocationType | undefined => {
